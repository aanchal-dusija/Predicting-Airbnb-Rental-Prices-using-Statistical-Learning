...
```{r}
set.seed(1234)
 alpha <- read.csv('/Users/aanchaldusija/Downloads/clean_airbnb.csv')
...
# Neighborhood is not a continuous number because it's a zip code, but the original model treats it as such. This is incorrect and we must treat it as a factor/fixed effect.
set.seed(2343)
...
set.seed(2121)
# ONLY NUMERIC
# You dont only have to include bs() numerical variables in the spline models. We can keep all our variables (including the correctly encoded neighbourhood variable) and allow nonlinearity in our numeric variables. 
...
dat1<- subset(alpha, select = -c(price))
...
```{r}
xgboost_data <- subset(alpha, select = -c(longitude, price, latitude, long_stay, old_listing, neighbourhood))
```
