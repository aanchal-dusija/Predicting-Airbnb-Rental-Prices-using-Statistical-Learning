...
dir = "/Users/aanchaldusija/Downloads/ANLY 512 Project/airbnb/"
...
```
```{r}
# Define custom marker color
my_color <- "black"

# Create the leaflet map with custom marker color
m <- leaflet(neighborhood) %>% 
  addProviderTiles(providers$CartoDB.Positron) %>%
  addCircleMarkers(~avg_lon, ~avg_lat,
             radius = ~med_price/50,
...
```{r}
ggplot(data  = neighborhood, aes(x = reorder(neighbourhood, med_price), y = med_price)) + 
  geom_bar(stat = "identity") +
  coord_flip()

hist(neighborhood$med_price)
#Looks like there are 2 neighborhoods that are much more expensive than the others(78656 & )
```
```{r}
library(ggplot2)
library(dplyr)
